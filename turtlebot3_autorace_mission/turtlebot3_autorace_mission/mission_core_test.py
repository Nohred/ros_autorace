...
class FollowLane(State):
    def __init__(self, node):
        ...
        self.REQUIRED_DURATION = {
            'sign_intersection': 3.0,
            'sign_construction': 0.6,
            'sign_parking': 2.5,
            'sign_level': 0.3,
            'sign_tunnel': 0.6,
        }
    ...
    def execute(self, sensor_data):
        ...
        if sensor_data['avoid_active']:
            return AvoidConstruction(self.node), Twist()

        step = self.node.mission_step

        # PASO 0: Interseccion -> Giro a la derecha (valor 3)
        if step == 0 and self._check_sign(sensor_data, 'sign_intersection', 3):
            self.node.mission_step = 1
            return IntersectionRight(self.node), Twist()

        # PASO 1: Construccion (via señal, por si el LIDAR no la agarra a tiempo)
        elif step == 1 and self._check_sign(sensor_data, 'sign_construction', 1):
            self.node.get_logger().info("Senal de CONSTRUCCION detectada visualmente.")
            self.node.mission_step = 2
            # No cambiamos de estado aqui; dejamos que el LIDAR (avoid_active) maneje el esquive fisico

        # PASO 2: Parking
        elif step == 2 and self._check_sign(sensor_data, 'sign_parking', 1):
        ...
        # PASO 3: Giro a la izquierda
        elif step == 3 and self._check_sign(sensor_data, 'sign_level', 2):
            self.node.mission_step = 4
            return LeftTurnStop(self.node), Twist()

        # PASO 4: Level Crossing -> Esperar barrera abierta
        elif step == 4 and self._check_sign(sensor_data, 'sign_level', 1):
            self.node.get_logger().info("Senal de CRUCE FERROVIARIO detectada.")
            return LevelCrossingMission(self.node), Twist()

        # PASO 5: Tunnel
        elif step == 5 and self._check_sign(sensor_data, 'sign_tunnel', 1):
...
class LevelCrossingMission(State):
    def __init__(self, node):
        super().__init__(node)
        self.confirmed_open_time = None
        self.CONFIRM_OPEN_SECONDS = 1.0  # Debe ver "abierto" continuo antes de avanzar
    ...
    def execute(self, sensor_data):
        ...
        if level_state == 2:  # STOP - barrera cerca, detenerse
            cmd_vel.linear.x = 0.0
            cmd_vel.angular.z = 0.0
            self.confirmed_open_time = None

        elif level_state == 1:  # SLOWDOWN - se acerca, avanzar despacio
            cmd_vel.linear.x = 0.03
            cmd_vel.angular.z = sensor_data['lane_cmd'].angular.z  # seguir dirigiendo con el PID
            self.confirmed_open_time = None

        elif level_state == 3:  # OPEN - via libre
            now = self.node.get_clock().now()
            if self.confirmed_open_time is None:
                self.confirmed_open_time = now

            elapsed = (now - self.confirmed_open_time).nanoseconds / 1e9
            cmd_vel = sensor_data['lane_cmd']  # avanzar normal mientras confirmamos

            if elapsed >= self.CONFIRM_OPEN_SECONDS:
                self.node.get_logger().info("Cruce despejado. Regresando a FOLLOW_LANE.")
                self.node.mission_step += 1
                return FollowLane(self.node), cmd_vel
        else:
            # Estado 0: aun no ve nada relevante, seguir linea normal
